...
if(!isNaN(cliente)){
    cliente = nomes[Number(cliente)-1];
}
  if(!fiado[cliente]){
   fiado[cliente] = 0;
  }
...
function fazerSangria(){
 ...
 let motivo = prompt("Motivo da sangria:");
 ...
 let registro = {
  ...
  operador: usuarioLogado || "Administrador"
 };

 sangrias.push(registro);
...
if (tela) {
    ...
    <body>
    <pre style="font-size:16px;font-family:monospace">${cupom}</pre>
...
function listarFiado(){
console.log(fiado);
console.log(Object.keys(fiado));

    let lista=document.getElementById("listaFiado");

    lista.innerHTML="";

    let busca=document.getElementById("pesquisaFiado").value.toLowerCase();

    let clientes=Object.keys(fiado).sort();

    clientes.forEach(cliente=>{

        if(!cliente.toLowerCase().includes(busca)) return;
        ...
        Devedor:
        <b style="color:red">
        R$ ${Number(fiado[cliente]).toFixed(2)}
        </b>
...
3 - Cartão`
 ...
 let nomeForma = "Dinheiro";

 if(forma == "2") nomeForma = "Pix";
 if(forma == "3") nomeForma = "Cartão";
 ...
 if(nomeForma == "Dinheiro") caixaResumo.dinheiro += valor;
 if(nomeForma == "Pix") caixaResumo.pix += valor;
 if(nomeForma == "Cartão") caixaResumo.cartao += valor;

 historico.push({
    itens:[
      {
        nome:"Recebimento de Fiado",
        codigo:"FIADO",
        qtd:1,
        preco:valor
      }
    ...
    total:valor,
    forma:nomeForma,
    pagamentos:[{tipo:nomeForma,valor:valor}],
    data:new Date().toISOString(),
    cliente:cliente,
    tipo:"Recebimento de Fiado"
 });
